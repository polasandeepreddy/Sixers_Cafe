...
const PaymentSummary: React.FC<PaymentSummaryProps> = ({ className = '', onPaymentComplete }) => {
  ...
  return (
    <div className={`max-w-3xl mx-auto bg-white rounded-xl shadow-lg p-6 grid grid-cols-1 md:grid-cols-2 gap-6 ${className}`}>
      ...
      <section className="space-y-4 flex flex-col items-center">
        <h2 className="text-2xl font-bold text-gray-800 border-b pb-2 mb-4 w-full text-center">
          Payment
        </h2>
        ...
        <div className="text-center text-gray-700 space-y-1 text-sm">
          <p><span className="font-semibold">UPI ID:</span> {upiDetails.upiId}</p>
          <p><span className="font-semibold">Amount:</span> ₹{upiDetails.amount}</p>
        </div>
        ...
        <p className="text-xs text-gray-400 text-center mt-3">
          Please ensure the screenshot clearly shows the transaction details. Your booking will be confirmed after admin verification.
        </p>
      </section>
    </div>
  );
